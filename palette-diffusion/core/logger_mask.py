    ...
    def save_images(self, results):
        ...
        try:
            ...
            for i in range(len(gt)):
                ...
                fig, axu = plt.subplots(1, 4)
                vmin = 0
                vmax = 255
                axu[0].imshow(gt[i][:, :], vmin=vmin, vmax=vmax, interpolation="none", cmap=mpl.colormaps["magma"])
                axu[0].set_title("Ground Truth U")
                axu[0].set_axis_off()
                axu[1].imshow(mask[i][:, :], vmin=vmin, vmax=vmax, interpolation="none", cmap=mpl.colormaps["magma"])
                axu[1].set_title("Mask U")
                axu[1].set_axis_off()
                axu[2].imshow(out[i][:, :], vmin=vmin, vmax=vmax, interpolation="none", cmap=mpl.colormaps["magma"])
                axu[2].set_title("Output U")
                axu[2].set_axis_off()
                axu[3].imshow(np.abs(out[i] - gt[i])[:, :], vmin=vmin, vmax=vmax, interpolation="none", cmap=mpl.colormaps["magma"])
                axu[3].set_title("Difference U")
                axu[3].set_axis_off()

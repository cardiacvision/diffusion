    ...
    def save_images(self, results):
        ...
        try:
            ...
            for i in range(len(gt)):
                fig, axu = plt.subplots(5, 3)
                for j in range(5):
                    vmin = 0
                    vmax = 255
                    axu[j][0].imshow(gt[i][:, :, j], vmin=vmin, vmax=vmax, interpolation="none", cmap=mpl.colormaps["magma"])
                    axu[0][0].set_title("Ground Truth U")
                    axu[j][0].set_axis_off()
                    axu[j][1].imshow(out[i][:, :, j], vmin=vmin, vmax=vmax, interpolation="none", cmap=mpl.colormaps["magma"])
                    axu[0][1].set_title("Output U")
                    axu[j][1].set_axis_off()
                    axu[j][2].imshow(np.abs(out[i] - gt[i])[:, :, j], vmin=vmin, vmax=vmax, interpolation="none", cmap=mpl.colormaps["magma"])

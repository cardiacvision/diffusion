    ...
    def save_images(self, results):
        ...
        try:
            ...
            for i in range(len(gt)):
                ...
                fig, axu = plt.subplots(2, 2)
                axu[0,0].imshow(gt[i][:, :, 0], vmin=0, vmax=255, interpolation="none", cmap=mpl.colormaps["magma"])
                axu[0,0].set_axis_off()
                axu[0,0].set_title("Ground Truth")
                axu[0,0].set_ylabel("U")
                axu[1,0].imshow(gt[i][:, :, 1], vmin=0, vmax=255, interpolation="none", cmap=mpl.colormaps["magma"])
                axu[1,0].set_axis_off()
                axu[1,0].set_ylabel("V")
                axu[0,1].imshow(out[i][:, :, 0], vmin=0, vmax=255, interpolation="none", cmap=mpl.colormaps["magma"])
                axu[0,1].set_axis_off()
                axu[0,1].set_title("Prediction")
                axu[1,1].imshow(out[i][:, :, 1], vmin=0, vmax=255, interpolation="none", cmap=mpl.colormaps["magma"])
                axu[1,1].set_axis_off()

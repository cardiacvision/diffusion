...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
        ...
        for cate_idx, subd in enumerate(self.subdirs):
            ...
            for x in os.listdir(sub_path):
                if not x.endswith('.npz'):
                    continue
                all_mids.append(os.path.join(self.split, x[:-len('.npz')]))

            # NOTE: [mid] contains the split: i.e. "train/<mid>" or "val/<mid>" or "test/<mid>"
            if self.split == "val":
                all_mids = all_mids[:128]
            for mid in all_mids:
                # obj_fname = os.path.join(sub_path, x)
                obj_fname = os.path.join(root_dir, subd, mid + ".npz")
                try:
                    point_cloud = np.load(obj_fname)  # (nk, 3)
                    point_cloud = np.concatenate([point_cloud["XYZ"], point_cloud["Voltage"]], axis=-1)
                    np.random.shuffle(point_cloud)
                    indices = np.random.choice(len(point_cloud), 20000, replace=False)
                    point_cloud = point_cloud[indices]

                except:
                    continue

                assert point_cloud.shape[0] == 20000
        ...
        self.tr_sample_size = min(40000, tr_sample_size)
        self.te_sample_size = min(40000, te_sample_size)
...
class Heart15kPointClouds(Uniform15KPC):
    def __init__(self, root_dir="/mnt/data1/shared/data/tanish-sph-v1",
                 categories=['FK'], tr_sample_size=4000, te_sample_size=1000,
        ...
        super(Heart15kPointClouds, self).__init__(
